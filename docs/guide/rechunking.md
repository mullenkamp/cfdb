...
The `groupby()` method builds on `iter_chunks()` and the rechunker. When passed coordinate names (e.g. `groupby('latitude')`), it sets chunk size 1 along the grouped dimensions. When passed a dict with time period strings (e.g. `groupby({'time': 'D'})`), it computes the appropriate chunk size or falls back to slice-based iteration for irregular periods like monthly or yearly. Similarly, `iter_chunks(chunk_shape)` delegates to the rechunker internally when a custom chunk shape is provided. The `Rechunker` class is useful when you need the planning methods (`calc_n_reads_rechunker`, `guess_chunk_shape`, etc.) or direct control over the rechunk operation. See [Data Variables — GroupBy](data-variables.md#groupby).
