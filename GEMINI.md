...
*   **Data Structure:**
    *   **Datasets:** Collections of variables. Four types, from the `data_models.Type` enum: `grid` (default, N-D gridded), `ts_ortho` (time series at point geometries), and — from cfdb 0.9.6 — `ts_forecast` `(point, forecast_reference_time, forecast_period)` and `grid_forecast` `(x, y, forecast_reference_time, forecast_period)`. The forecast pair indexes by *lead* rather than valid time, because a valid-time axis would be ~97% empty. ⚠️ **The enum lives in the separate `cfdb-models` distribution**, so adding a type needs a cfdb-models release first. ⚠️ **Most `dataset_type` checks in the codebase are exact string equality**, so a new type inherits nothing automatically — the one exception is `utils.parse_coord_inputs`, which tests `'ts_' in dataset_type`.
    *   **Coordinates:** 1D, unique, sorted, non-null arrays defining dimensions (e.g., lat, lon, time, or `forecast_reference_time`/`forecast_period` for the forecast types). At most one coordinate may carry each CF axis (X/Y/Z/T), which is why `forecast_period` deliberately carries none. Values cannot be changed in-place; supports `append()`, `prepend()`, and `truncate(start, stop)` to grow or shrink. There is no insert operation. When a coordinate has an enforced `step` and new data is appended/prepended with a gap, cfdb auto-fills the missing intermediate values (if the gap is a valid multiple of the step). Data variables at filled positions are empty until written.
...
*   **Rechunker Storage-Space Declaration (2026-07 perf round):** the `Rechunker` declares the array to `rechunkit` in a declared space shifted by `d = origin % chunk_shape` per dim (`decl_shape = full_shape + d`, `sel_decl = sel + d`, always materialized — never `None` when `d != 0`), so storage-chunk boundaries land on multiples of `chunk_shape` and every rechunkit read maps to exactly ONE storage chunk (coordinate origins ride rechunkit's phase machinery). The source functions convert declared→storage with a constant shift (a multiple of `chunk_shape`) and keep the multi-chunk assembly loop as a tested defensive fallback — do not delete it, and do not "simplify" the source function to `get_chunk()`. Yields are selection-relative and byte-identical to the pre-transform implementation.
