...
*   **Reference cycles and `weakref.finalize`:** `Dataset` uses `weakref.finalize` for cleanup (flushing metadata, releasing file locks). Any class stored as an attribute on `Dataset` or `Variable` that holds a reference back to it **must** use `weakref.proxy(dataset)` — not a direct reference. A strong back-reference creates a cycle that prevents the finalizer from running on Python 3.12+, causing file locks to persist and silent hangs during GC/teardown. This applies to `Creator`/`Coord`/`DataVar`/`CRS` (in `creation.py`) and `LocationIndexer` (in `indexers.py`).
